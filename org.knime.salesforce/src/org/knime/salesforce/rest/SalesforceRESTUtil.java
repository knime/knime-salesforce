...
public final class SalesforceRESTUtil {
    ...
    public static <R> R doGet(final URI uri, final SalesforceAuthentication auth, final boolean refreshTokenIff,
        final Timeouts timeouts, final Functions.FailableFunction<Response, R, SalesforceResponseException> callback)
            throws SalesforceResponseException {
        ...
        try (Response response = client.get()) {
            ...
            if (response.getStatus() == Status.UNAUTHORIZED.getStatusCode() && refreshEnabled) {
                ...
                response.close();
                return doGet(uri, newAuth, false, timeouts, callback);
            }
    ...
    private static WebClient getClient(final URI uri, final SalesforceAuthentication auth, final Timeouts timeouts) {
        ...
        httpConduit.getClient().setConnectionTimeout(1000L * timeouts.getConnectionTimeoutS());
        httpConduit.getClient().setReceiveTimeout(1000L * timeouts.getReadTimeoutS());
    ...
    public static SObject[] getSObjects(final SalesforceAuthentication auth, final Timeouts timeouts)
        ...
        return doGet(uri, auth, true, timeouts, response -> checkResponse(response, SObjects.class).getSobjects());
    }
    ...
    public static Field[] getSObjectFields(final SObject object, final SalesforceAuthentication auth,
        ...
        return doGet(uri, auth, true, timeouts,
            response -> checkResponse(response, SObjectDescription.class).getFields());
    }
